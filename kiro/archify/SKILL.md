...
3. Artifact first: the next tool action must write the candidate. Write the candidate before inspecting renderer internals. Do not plan exact coordinates in prose. Start with one clear main path, short side branches, sparse labels, and at most 12 primary nodes. Set `meta.quality_profile` to `"showcase"` unless the user explicitly requests a dense `standard` map. Start with automatic routes and labels. Do not add `via`, `channelX`, `channelY`, or `labelAt` before a diagnostic calls for one; apply at most one diagnosed geometry control per repair. **Exception — low-level design (LLD) / system architecture diagrams:** the 12-node cap and the showcase-first default do not apply. Read the `lld-diagram-standard` skill (user standing rule, added 2026-09-11) before authoring one: pull every named component from the project's own design doc/spec (do not thin it out), expect 25-40+ nodes to be normal, target `meta.quality_profile: "standard"` from the start (showcase's absolute font-size floor is usually infeasible at this density), and lay the diagram out as a tall/narrow vertical spine rather than a wide row — width, not height, is what fails the 1440px desktop-readability check.
